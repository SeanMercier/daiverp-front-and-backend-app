...
token_request = urllib2.Request("http://169.254.169.254/latest/api/token", method='PUT', headers={'X-aws-ec2-metadata-token-ttl-seconds': 21600})
token = urllib2.urlopen(token_request).read().decode(encoding)

#Instance information
BASE_URL = 'http://169.254.169.254/latest/meta-data/'
instanceid_request = urllib2.Request(BASE_URL + 'instance-id', headers={'X-aws-ec2-metadata-token': token})
INSTANCE_ID = urllib2.urlopen(instanceid_request).read().decode(encoding)
imageid_request = urllib2.Request(BASE_URL + 'ami-id', headers={'X-aws-ec2-metadata-token': token})
IMAGE_ID = urllib2.urlopen(imageid_request).read().decode(encoding)
instancetype_request = urllib2.Request(BASE_URL + 'instance-type', headers={'X-aws-ec2-metadata-token': token})
INSTANCE_TYPE = urllib2.urlopen(instancetype_request).read().decode(encoding)
